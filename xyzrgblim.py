...
for i in range(1,number_of_stars):
	...
	if rgb[0] == -1: #bv_to_rgb mapping fails sometimes. if it does, move on to next star
		continue
	if float(data[4]) > 100.0 or i == 1: #make small stars into points instead of spheres (except for the Sun), for performance
		newsphere = sphere()
	...
	output.write(string) #write python-array-formatted data to output file
	output.write("\n")

	#skip a bunch of lines to get a more evenly spaced data set (stars.speck contains ~ 100000 stars)
	for x in xrange(1,100000/number_of_stars):
		stars_speck.readline()

# failed attempts to do a flythrough
# while 1:
